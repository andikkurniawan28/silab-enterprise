...
@section('material_category-active')
    {{ 'active' }}
@endsection
                        ...
                        <tbody>
                            @foreach ($material_categories as $material_category)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group" role="group" aria-label="manage">
                                            <a href="{{ route('material_category.edit', $material_category->id) }}"
                                                class="btn btn-secondary btn-sm">Edit</a>
                                            <button type="button" class="btn btn-danger btn-sm delete-btn"
                                                data-id="{{ $material_category->id }}"
                                                data-name="{{ $material_category->name }}">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.addEventListener('click', function(event) {
                if (event.target.classList.contains('delete-btn')) {
                    ...
                    const material_category_id = button.getAttribute('data-id');
                    const material_category_name = button.getAttribute('data-name');
                    Swal.fire({
                        title: 'Are you sure?',
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.setAttribute('action',
                                `{{ route('material_category.destroy', ':id') }}`.replace(
                            ...
                            name.setAttribute('value', material_category_name);
